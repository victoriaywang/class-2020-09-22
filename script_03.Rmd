...
sep_old <- 
  ...
  summarize(total_enroll = sum(u_grad), .groups = "drop") %>%
...
```{r}
sep_new <- read_excel("new_data/class_enrollment_summary_by_term_9-22-2020.xlsx", skip = 3) %>% 
  clean_names() %>% 
  filter(! is.na(course_title)) %>% 
  select(-grad, -non_degree, -x_reg, -vus, -employee, 
         -withdraw, -total, -instructor_full_name, -course_section_code) %>%
...
- account for changes in total undergraduate enrollment in affecting course size



After we have a model, Temperance suggests we should:
- unsure if data is up to date
- acknowledge unprecedented situation
- can't compare to previous years
...
enrollment_change <- left_join(sep_old, sep_new, by = "id", suffix = c("_old", "_new")) %>%
  mutate(change_in_enrollment = total_enroll_new - total_enroll_old) %>%
  arrange(change_in_enrollment) %>%
  drop_na() #%>%
  #slice(1:5, 380:384)

View(enrollment_change)

ggplot(enrollment_change, aes(x = fct_reorder(title_new, change_in_enrollment), y = change_in_enrollment)) + 
    geom_col() + 
    coord_flip() + 
    labs(title = "Change in Enrollment", 
